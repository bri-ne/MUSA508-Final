...
train.weather.panel %>%
  ...
  summarize(Percipitation = mean(lag4Hourswind)) %>%
    ggplot(aes(y_numeric, Percipitation)) + geom_bar(stat = "identity") +
...
Model3<- glm(y_numeric ~ .,
                        data=dataTrain %>% 
                          dplyr::select(y_numeric,
                                        from, 
                                        intervalhour,
                                        lagHour, lag2Hours, lag3Hours), #### SOME VARS NEED INTERVAL HOUR THOugh
                        family="binomial" (link="logit"))
...
Model4<- glm(y_numeric ~ .,
                        data=dataTrain %>% 
                          dplyr::select(y_numeric,
                                        ...
                                        lag12Hoursrain, lag12Hourswind, lag12Hourstemp,lag2Hoursrain, lag2Hourswind, lag2Hourstemp, line), #### SOME VARS NEED INTERVAL HOUR THOugh
                        family="binomial" (link="logit"))
```
