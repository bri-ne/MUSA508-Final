...
bergInterval <- berg%>%group_by(stop_sequence, from, intervalhour)%>%summarize(late = mean(delay_dt, na.rm=T))%>%arrange(intervalhour)%>% 
                                                                                mutate(lagHour = dplyr::lag(late,1),
                                                                                         ...
                                                                                         lag1day = dplyr::lag(late,24)) ## This is a Study Panel               
...
glad <- train.template%>%filter(line == "Gladstone Branch")
  ...
  # grouping by all the things we want to investigate later, so we don't lose detail: 
gladInterval <- glad%>%group_by(stop_sequence, from, intervalhour)%>%summarize(late = mean(delay_dt, na.rm=T))%>%arrange(intervalhour)%>%
                                                                                mutate(lagHour = dplyr::lag(late,1),
                                                                                         lag2Hours = dplyr::lag(late,2),
                                                                                         lag3Hours = dplyr::lag(late,3),
                                                                                         lag4Hours = dplyr::lag(late,4),
...
ggplot()+
  geom_sf(data=bergGEOM, aes(color = late))




bergGEOM <- bergGEOM%>%mutate(lateM = late/60,
...
test<-bergGEOM%>%dplyr::filter(hour == 13 & week ==2)
...
basemap_ggplot(bergGEOM, map_service = "osm_stamen", map_type = "toner")

bergMAP2<- ggplot()+
  geom_sf(data=njGEO, fill="black")+
geom_sf(data = bergGEOM%>%dplyr::filter(hour == 13 & week ==2),
          shape=21, color= 'grey10', alpha = 0.6,
          aes(size = late, fill=late))+
  #scale_size_manual(name = "Lateness",
                    #labels = bergGEOM, "late")+
  scale_fill_gradient( low = "#260a03",
                          high = "#a81010",
                          space = "Lab",
                          na.value = "grey50",
                          #guide = "colourbar",
  ...
  labs(title = "Mean Lateness By Stop", 
...
bergMAP<- ggplot()+
  geom_sf(data=njGEO, fill="grey80")+
geom_sf(data = bergGEOM%>%dplyr::filter(hour == 13 & week ==2),
          shape=21, color= 'grey10', alpha = 0.6,
          aes(size = q5(lateM),
              fill= q5(lateM)))+
  ...
  labs(title = "Minutes Late By Stop 9:00 AM Hour ET", 
       #subtitle = "By Tract, Boston 2013-2018",
       caption = "Figure 3.1")+
  guides(size = guide_legend(override.aes = list(fill = brewer.pal(5, name="BuPu"))
    ))+
      facet_wrap(~ weekday)#+mapTheme()
...
testProbs4 <- data.frame(Outcome = as.factor(dataTest$y2_numeric),
...
cvFit4 <- caret::train(y2 ~ .,
                       data=dataTrain %>%dplyr::select(all_of(Model4.vars), y2), 
                       method="glm", family="binomial",
                       metric="ROC", trControl = ctrl)
