...
set.seed(2121)
panelnoGEOMTrain <- filter(panelnoGEOM, week < 2)
...
ModelcvFit1 <- caret::train(y2 ~ .,
                        data=panelnoGEOMTrain %>% 
                        na.exclude(  
                          dplyr::select(all_of(Model1.vars), y2)), ### in the book, they cv on all the data but i'm pretty sure it should just be train
...
ModelcvFit2 <- caret::train(y2 ~ .,
                        data=panelnoGEOMTrain %>% 
                        na.exclude(  
                          dplyr::select(all_of(Model2.vars), y2)), ### in the book, they cv on all the data but i'm pretty sure it should just be train
                        method="glm", family="binomial",
                        metric="ROC", trControl = ctrl)
...
plot_grid(ModelcvFit1.plot, ModelcvFit2.plot, nrow=2, ncol=1)
